...
usage() {
    ...
    echo "  --ou <value>          Specify organizational unit for certificates (default: 'ProductCopy')"
    echo "  --email <value>       Specify email for certificates (default: 'simon@storizzi.com')"
    echo ""
...
generate_ca() {
    ...
    openssl req -x509 -new -key "$CA_KEY" -out "$CA_CERT" -days 365 \
        -subj "/C=$COUNTRY/ST=$STATE/L=$CITY/O=$ORG/OU=$OU/CN=Storizzi-CA/emailAddress=$EMAIL"
}
...
generate_client_cert() {
    ...
    openssl req -new -key "$CLIENT_KEY" -out "$CLIENT_CSR" \
        -subj "/C=$COUNTRY/ST=$STATE/L=$CITY/O=$ORG/OU=$OU/CN=Storizzi-Client/emailAddress=$EMAIL"
